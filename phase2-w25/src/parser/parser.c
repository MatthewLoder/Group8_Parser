...
// Get next token
static void advance(void) { current_token = get_next_token(source, &position); }
...
static int match(TokenType type) { return current_token.type == type; }
...
// Main parse function
ASTNode *parse(void) { return parse_program(); }
